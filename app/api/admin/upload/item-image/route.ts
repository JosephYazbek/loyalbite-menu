...
function buildFilePath(restaurantId: string, itemId: string, mimeType: string) {
  const extension =
    mimeType === "image/png"
      ? "png"
      : mimeType === "image/webp"
  ...
  return `restaurant_${restaurantId}/item_${itemId}.${extension}`;
}
...
export async function POST(req: Request) {
  try {
    ...
    const filePath = buildFilePath(restaurantId, itemId, file.type);
    ...
    if (oldPath) {
      ...
      if (deleteError) {
        console.error("[UPLOAD][item] remove error", deleteError);
        return NextResponse.json(
    ...
    if (uploadError) {
      console.error("[UPLOAD][item] uploadError", uploadError);
      return NextResponse.json(
    ...
    console.error("[UPLOAD][item] unexpected error", error);
    const message =
