...
const breakdownLabels: Record<
  keyof MenuHealthScore["breakdown"],
  { label: string; hint: string }
> = {
  photos: {
    label: "Photos",
    hint: "Items with appetizing photos get more taps.",
  },
  descriptions: {
    label: "Descriptions",
    hint: "Short sentences help guests imagine the dish.",
  },
  translations: {
    label: "Translations",
    hint: "Support both English and Arabic diners.",
  },
  tags: {
    label: "Tags",
    hint: "Highlight dietary info or selling points.",
  },
  structure: {
    label: "Structure",
    hint: "Balanced categories keep menus scannable.",
  },
  performance: {
    label: "Performance",
    hint: "Popular items get consistent views.",
  },
};

const severityStyles: Record<
  MenuHealthIssue["severity"],
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div>
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Based on photos, descriptions, translations, structure, tags, and recent guest
              engagement.
            </p>
          </div>
      ...
      <section className="space-y-4 rounded-3xl border border-border bg-card p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              See how each dimension contributes to your score.
            </p>
          </div>
        </div>
      ...
      <section className="space-y-4 rounded-3xl border border-border bg-card p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Sorted by severity – tackle the big wins first.
            </p>
          </div>
        </div>
        {health.issues.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/80 bg-secondary/40 px-4 py-6 text-center text-sm text-muted-foreground">
            Your menu looks great! Keep adding photos and testing new dishes for even better
            performance.
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-semibold ${severity.badge}`}
                    >
                  ...
                  {issue.hint && (
                    <p className={`mt-2 text-xs ${severity.text}`}>{issue.hint}</p>
                  )}
                </div>
              );
            })}
